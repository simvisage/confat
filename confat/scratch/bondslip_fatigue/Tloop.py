    ...
    def eval(self):
        ...
        while t_n1 <= self.t_max - self.d_t:
            ...
            while k <= self.k_max:
                ...
                R, K, eps, sig, xs_pi, alpha, z, w = self.ts.get_corr_pred(
...
#                 print 'r', np.linalg.norm(R)
            ...
            #print'K=',k
        return U_record, F_record, sf_record, np.array(t_record), eps_record, sig_record, w_record
...
# BCDof(var='u', dof=n_dofs - 1, value=2.5, time_function=tf)]
    ...
    ts.bc_list = [BCDof(var='u', dof=0, value=0.0),
                      BCDof(var='u', dof=n_dofs - 1, value = 0.1)]
